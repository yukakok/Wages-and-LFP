...
#Note: there are a lot of observations with 0 wage regardless of their positive inctot             However, excluding obs with 0 hourly wage and leaving obs with 0.0002 hourly wage           doesn't make much sense, so I will use all values to calculate the mean unless they         have NA here
...
#Based on the deifinition of the labor force, I include armed forces into LFP (originally having missing values)
cps_lf$lfp[is.na(cps_lf$lfp)] <- 1

#Create education category for college experiences - no(high school), some, or BA and over. For the education measurement before 1992, I consider 4 years of college equivalent to BA
...
#Get CPI99 values from here (https://cps.ipums.org/cps/cpi99.shtml) to take inflation into account. Using these values, I convert nominal hourly wage to 1999 dollars first (nominal wage*CPI99) and then to 2017 dollars by multiplying 1.471
...
```{r preparation2}
#Here, I will only use male aged between 25 and 64 (prime working-age people) and obtain the rate for each skilled group (skill is defined by the education level)
over25 <- cps_lf %>%
...
```{r}
#The following code is to produce the theoretical demand and supply curve. I adjusted the number just to support my explanation, so the numbers are completely arbitrary and does not have any mean
